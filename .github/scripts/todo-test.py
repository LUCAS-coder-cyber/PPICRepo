...
if __name__ == "__main__":
    ...
    # Capture the relevant output (test names + status)
    output_lines = captured_output.getvalue().splitlines()

    # Print the output of each test with the status "OK"
    for line in output_lines:
        if "ok" in line:
            # Remove class name and method name from the line, keeping only the test name
            print(line.split(" ")[0] + " - ok")

    # Print total, passed, and failed summary
    total_tests = result.testsRun
    failed_tests = len(result.failures + result.errors)
